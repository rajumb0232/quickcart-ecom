...
/**
 * Manages Keycloak admin access tokens with automatic refresh.
 * <p>
 * This service provides thread-safe token retrieval and scheduled background refresh
 * to ensure admin tokens are always available for Keycloak administrative operations.
...
public class AdminTokenService {
    ...
    /**
     * Fetches a new token from Keycloak and updates the token storage.
     * <p>
    ...
    private synchronized void fetchTokenAndSet() {
        log.debug("Fetching token from Keycloak for realm='{}' client='{}'", props.getRealm(), props.getAdminClientId());
    ...
    /**
     * Performs the actual HTTP request to fetch a new admin token from Keycloak.
     * <p>
     * Uses client credentials grant type to authenticate with Keycloak and obtain
