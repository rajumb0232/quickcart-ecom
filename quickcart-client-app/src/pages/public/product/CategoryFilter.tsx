  ...
  function handlePick(item: Category) {
    if (item.category_level === level) {
      if (level === 1) {
        ...
        setSelL3(item);
        const names = [selL1?.name, selL2?.name, item.name].filter(
          Boolean
        ) as string[];
        onConfirm(names);
        onToggle();
    ...
    <div className="mb-4">
      ...
      <div>
        <button
          ...
          className="w-full text-left border border-gray-300 rounded-md px-3 py-2 flex items-center justify-between"
          disabled={isLoading}
        >
          <span className="text-sm">
            {isLoading ? "Loading..." : pathLabel}
          </span>
          <span className="text-gray-600">{isOpen ? "▴" : "▾"}</span>
        </button>

        {isOpen && (
          <div className="mt-2 w-full bg-white border border-gray-200 rounded-md shadow-sm p-3">
                  ...
                  {currentList.map((it) => (
                    <li
                      ...
                      className={`px-3 py-3 rounded cursor-pointer text-sm mb-1 ${
                        (level === 1 &&
                          selL1?.category_id === it.category_id) ||
                        (level === 2 &&
                          selL2?.category_id === it.category_id) ||
