...
export const CategoryFilter: React.FC<CategoryFilterProps> = ({
  ...
  useEffect(() => {
    ...
    if (resolvedCats && resolvedCats.length > 0) updateAllCategories(resolvedCats);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [location.search]);
  ...
  const currentList =
    level === 1 ? categories : level === 2 ? selL1?.child_category : selL2?.child_category;
  const currentTitle =
    level === 1 ? "Select Categories" : level === 2 ? "Select in " + selL1?.name : "Select in " + selL2?.name;
          ...
          <div className="absolute left-0 right-0 bg-white border border-gray-200 rounded-md shadow-sm p-3 mt-1 z-20">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex gap-2">
                <button onClick={goBack} className="text-sm px-2 py-1 border rounded text-gray-700">
                  <IoCaretBackOutline />
                </button>
                <button onClick={goForward} className="text-sm px-2 py-1 border rounded text-gray-700">
                  <IoCaretForward />
                </button>
                <button onClick={resetAll} className="text-sm px-2 py-1 border rounded text-gray-500">
                  <MdDelete />
                </button>
              </div>
            </div>
                  ...
                  {currentList.map((it) => {
                    ...
                    return (
                      <li
                        ...
                        className={`px-3 py-3 rounded cursor-pointer text-sm mb-1 ${isActive ? "bg-green-100" : "hover:bg-gray-50"}`}
                      >
                        {it.name}
                      </li>
                    );
