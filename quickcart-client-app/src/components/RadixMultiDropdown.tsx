      ...
      return (
        <div key={`${item.name}-${idx}`}>
          ...
          >
            ...
            >
              <button
                className="w-full text-left px-4 py-3 text-sm flex items-center hover:bg-slate-50 cursor-pointer outline-none border-none"
                onClick={(e) => {
                  e.preventDefault();
                  item.onClick?.();
                }}
            ...
            <DropdownMenu.Portal>
              ...
              >
                {item.child!.map((child, i) => (
                  <DropdownMenu.Item
                    key={`${child.name}-${i}`}
                    className="w-full text-left px-4 py-3 text-sm hover:bg-slate-50 flex items-center cursor-pointer outline-none border-none"
                    onSelect={() => {
                      child.onClick?.();
                      closeAll();
                    }}
